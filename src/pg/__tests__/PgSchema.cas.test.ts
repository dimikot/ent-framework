...
test("CAS batched success and skip for the same row but different $cas values", async () => {
...
test("CAS disables coalescing for the same row with the same $cas values", async () => {
  const id = nullthrows(
    await shardRun(shard, schema.insert({ name: "a", ts: null })),
  );
  master.resetSnapshot();
  ...
  const rows = await join([shardRun(shard, schema.load(id))]);
  expect(rows).toMatchObject([{ id, name: "a-upd" }]);
});
