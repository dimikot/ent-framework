...
const schemaNullableUniqueKey = new PgSchema(
  'pg-schema.generic"table_nullable_unique_key',
  {
    ...
    url_name: { type: String, allowNull: true, autoInsert: "NULL" },
    created_at: { type: Date, autoInsert: "now()" },
    updated_at: { type: Date, autoUpdate: "now()" },
  },
  ["url_name"],
...
const schemaUniqueKeyWithAutoInsert = new PgSchema(
  'pg-schema.generic"table_unique_key_with_auto_insert',
  {
    id: { type: String, autoInsert: "id_gen()" },
    name: { type: String, autoInsert: "'myname'" },
    url_name: { type: String, autoInsert: "'n'" },
  },
  ["url_name"],
);
...
const schemaIDUniqueKey = new PgSchema(
  'pg-schema.generic"table_id_unique_key',
  {
    id: { type: String, autoInsert: "id_gen()" },
    name: { type: String },
  },
...
const schemaNameWithAutoInsert = new PgSchema(
  'pg-schema.generic"table_name_with_auto_insert',
  {
    id: { type: String, autoInsert: "id_gen()" },
    name: { type: String, allowNull: true, autoInsert: "'n'" },
    url_name: { type: String },
  },
  ["url_name"],
);
...
beforeEach(async () => {
  await recreateTestTables([
    ...
    },
    {
      ...
      SCHEMA: schemaUniqueKeyWithAutoInsert,
      SHARD_AFFINITY: [],
    },
    {
      CREATE: [
    ...
    },
    {
      ...
      SCHEMA: schemaNameWithAutoInsert,
      SHARD_AFFINITY: [],
    },
  ]);
...
test("upsert single", async () => {
  ...
  const id4 = await shardRun(
    shard,
    schemaIDUniqueKey.upsert({ id: id1, name: "a" }),
  );
  master.toMatchSnapshot();
...
test("upsert batched with nullable unique key must treat nulls as non-equal", async () => {
  ...
  const [id4] = await join([
    shardRun(
      shard,
      schemaNullableUniqueKey.upsert({ name: "ddd", url_name: null }),
    ),
  ]);
  expect(id4 !== id3).toBeTruthy();
  ...
  expect(rows).toMatchObject([
    ...
    { id: id4, name: "ddd", url_name: null },
  ]);
});

test("upsert with optional nullable unique key", async () => {
  const id0 = await shardRun(
    shard,
    schemaNullableUniqueKey.upsert({
      name: "000",
      // no url_name since it has autoInsert and can be omitted
    }),
  );
  const id1 = await shardRun(
    shard,
  ...
  expect(rows).toMatchObject([
    { id: id0, name: "000", url_name: null },
    { id: id1, name: "111", url_name: null },
  ]);
});

test("upsert with autoInsert name creates it", async () => {
  const id0 = await shardRun(
    shard,
    schemaUniqueKeyWithAutoInsert.upsert({
      // no name since it has autoInsert and can be omitted
      // no url_name since it has autoInsert and can be omitted
    }),
  );
  master.toMatchSnapshot();
  const rows = await master.rows(
    "SELECT id, name, url_name FROM %T ORDER BY name",
    schemaUniqueKeyWithAutoInsert.name,
  );
  expect(rows).toMatchObject([{ id: id0, name: "myname", url_name: "n" }]);
});

test("upsert with autoInsert name does not override on update when name is omitted", async () => {
  const id0 = await shardRun(
    shard,
    schemaUniqueKeyWithAutoInsert.upsert({
      name: "000",
  ...
  expect(rows).toMatchObject([{ id: id0, name: "000", url_name: "n" }]);
  expect(id0).toEqual(id1);
});

test("upsert does not override autoInsert field when it updates", async () => {
  const id0 = await shardRun(
    shard,
    schemaNameWithAutoInsert.upsert({ name: "my", url_name: "aaa" }),
  );
  const id1 = await shardRun(
    shard,
    schemaNameWithAutoInsert.upsert({ url_name: "aaa" }),
  );
  master.toMatchSnapshot();
  expect(id0).toEqual(id1);

  const [id2, id3] = await join([
    shardRun(
      shard,
      schemaNameWithAutoInsert.upsert({ name: "my", url_name: "bbb" }),
    ),
    shardRun(shard, schemaNameWithAutoInsert.upsert({ url_name: "bbb" })),
  ]);
  master.toMatchSnapshot();
  ...
  const id4 = await shardRun(
    shard,
    schemaNameWithAutoInsert.upsert({ url_name: "ccc" }),
  );
  const id5 = await shardRun(
    shard,
    schemaNameWithAutoInsert.upsert({ name: "my", url_name: "ccc" }),
  );
  master.toMatchSnapshot();
  expect(id4).toEqual(id5);

  const rows = await master.rows(
    "SELECT id, name, url_name FROM %T ORDER BY name",
    schemaNameWithAutoInsert.name,
  );
  expect(rows).toMatchObject([
    { id: id0, name: "my", url_name: "aaa" },
    { id: id2, name: "my", url_name: "bbb" },
    { id: id4, name: "my", url_name: "ccc" },
  ]);
});
