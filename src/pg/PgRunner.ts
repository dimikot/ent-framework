...
> extends Runner<TInput, TOutput> {
  ...
  declare ["constructor"]: typeof PgRunner;
  ...
  protected fmt(
    ...
    return template.replace(
      /%(?:T|SELECT_FIELDS|FIELDS|UPDATE_FIELD_VALUE_PAIRS|WHERE_FIELD_VALUE_PAIRS|PK)(?:\(([%\w,]+)\))?/g,
      (c: string, a?: string) => {
        ...
        // Comma-separated list of the passed fields (never with AS clause). If
        // `autos` is passed, then the corresponding SQL from `autos` is used
        // instead of the field's name if that field is in `autos`.
        if (c.startsWith("%FIELDS")) {
          assert(args.fields, `BUG: no args.fields passed in ${template}`);
          return args.fields
            .map(
              (field) =>
                args.autos?.[typeof field === "string" ? field : field.field] ??
        ...
        // - when a="X": field1=X.field1, field2=X.field2, ...
        // - when a="X,Y": X.field1=Y.field1, X.field2=Y.field2, ...
        // - If `autos` is passed, then the corresponding SQL from `autos` is
        //   used instead of the field's name on the right side of "=" if that
        //   field is in `autos`.
        // - %WHERE_FIELD_VALUE_PAIRS is the same, but uses "AND" as a
        //   separator, not ",".
        if (
          c.startsWith("%UPDATE_FIELD_VALUE_PAIRS") ||
          c.startsWith("%WHERE_FIELD_VALUE_PAIRS")
        ) {
          assert(args.fields, `BUG: no args.fields passed in ${template}`);
          assert(
            a,
            "BUG: you must pass an argument, source table alias name or a comma-separated pair of aliases",
          );
          const [left, right] = a.includes(",")
            ? a.split(",").map((s) => s.trim())
            : [undefined, a];
          return args.fields
            .map(
              (field) =>
                this.escapeField(field, { withTable: left }) +
                "=" +
                (args.autos?.[
                  typeof field === "string" ? field : field.field
                ] ?? this.escapeField(field, { withTable: right })),
            )
            .join(c.startsWith("%UPDATE_FIELD_VALUE_PAIRS") ? ", " : " AND ");
        }
  ...
  protected createWithBuilder({
    ...
    const [preSuffix, postSuffix] = suffix.startsWith(",")
      ? [",", suffix.substring(1).replace(/^\s*\n/s, "")]
      : ["", suffix];
  ...
  protected createValuesBuilder<TInput extends object>({
    ...
    const rowFunc = this.newFunction(
      ...
      "return " +
        ...
        (withKey
          ? (cols.length > 0 ? ' + ", "' : "") + " + this.escapeString($key)"
          : "") +
        ' + ")"',
    );
