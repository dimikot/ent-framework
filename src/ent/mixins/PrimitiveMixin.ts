...
>(
  ...
  class PrimitiveMixin extends Base {
    declare ["constructor"]: typeof PrimitiveMixin;
    declare readonly vc: VC;
    ...
    static async insertIfNotExists(
      ...
      if (this.TRIGGERS.hasInsertTriggers() || this.INVERSES.length > 0) {
        ...
        vc.cache(IDsCacheUpdatable).add(this, id2); // to enable privacy checks in beforeInsert triggers
    ...
    static async upsert(vc: VC, input: InsertInput<TTable>): Promise<string> {
      ...
      vc.cache(IDsCacheUpdatable).add(this, id);
      return id;
    }
    ...
    async updateOriginal(
      ...
      return this.constructor.TRIGGERS.wrapUpdate(
        async (input) => {
          ...
          if (updated) {
            this.vc.cache(IDsCacheUpdatable).add(this.constructor, this[ID]);
            await mapJoin(
    ...
    private static async createEnt(
      ...
      ent.vc.cache(IDsCacheReadable).add(this, ent[ID]);
      if (vc !== ent.vc) {
        vc.cache(IDsCacheReadable).add(this, ent[ID]);
      }
