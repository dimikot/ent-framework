...
const entTestUniverseSchema = new PgSchema(
  'ent.shards"universe',
  {
...
class EntTestUniverse extends BaseEnt(testCluster, entTestUniverseSchema) {
...
const entTestHumanSchema = new PgSchema(
  'ent.shards"human',
  {
    id: { type: ID, autoInsert: "id_gen()" },
    parent_id: { type: ID },
...
class EntTestHuman extends BaseEnt(testCluster, entTestHumanSchema) {
  static readonly CREATE = [
    `CREATE TABLE %T(
