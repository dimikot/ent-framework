...
export class VC {
   ...
   */
  async saveTimelines(storage: TimelineStorage): Promise<string | undefined> {
    if (!this.isLoggedIn()) {
      throw new Error(
        `One does not simply save timelines for a non-logged VC: ${this.toString()}`,
      );
    }

    const dataStr = this.serializeTimelines();
    if (!dataStr) {
      return undefined;
    }

    const needsSave = [...this.timelines.values()].some(
      (timeline) => !timeline.isSaved(),
    );
    if (needsSave) {
      // Save to the storage only if there were some changes in timelines. In
      // most of the cases (read-only requests) there are no changes, so we save
      // one write operation.
      await storage.save(this.principal, dataStr);
   ...
   */
  async loadTimelines(storage: TimelineStorage): Promise<VC> {
    if (!this.isLoggedIn()) {
      throw new Error(
        `One does not simply load timelines for a non-logged VC: ${this.toString()}`,
      );
    }

    const dataStrs = await storage.load(this.principal);
    const vc = this.deserializeTimelines(...dataStrs);
    for (const timeline of vc.timelines.values()) {
      timeline.setIsSaved();
    }
   ...
   */
  withoutFlavor(
    ...flavorClasses: Array<new (...args: never[]) => VCFlavor>
  ): this {
    if (!flavorClasses.length) {
      return this;
    }

    const newFlavors = new Map(this.flavors);
    let removed = false;

    for (const flavorClass of flavorClasses) {
      if (newFlavors.delete(flavorClass)) {
        removed = true;
      }
    }
