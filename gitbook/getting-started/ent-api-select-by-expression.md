...
This `???`, dear friends, is the **automatic database index** (or an index prefix, which is `parent_id` in the example). In fact, as we hinted above, without such an index, the queries will just blow up.&#x20;
